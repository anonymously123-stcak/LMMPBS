...
def train(
    ...
    def compute_metrics(eval_preds):
        ...
        reduced_loss = -((pre[1]*np.log(pre[0]) + (1-pre[1])*np.log(1-pre[0]))).mean()
        return {'auc': auc, "reduced_loss":reduced_loss}
    ...
    def preprocess_logits_for_metrics(logits, labels):
        ...
        """
        logits = torch.softmax(logits[:, -3, [8241, 3782]], dim=-1)[:, 0]
        gold = (labels[:, -2] == 8241).float()
        return logits, gold
