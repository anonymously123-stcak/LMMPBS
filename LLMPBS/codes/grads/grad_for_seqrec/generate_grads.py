...
import torch
from grads_utils import Grads_Trainer, GradLlamaForCausalLM
...
def get_grads(args):
    ...
    model = GradLlamaForCausalLM.from_pretrained(
        base_model,
        load_in_8bit=True,
    ...
    trainer = Grads_Trainer(  
        model=model,  
        train_dataset=train_data,  
        eval_dataset=train_data,  
        gradspath=args.gradspath,  
        args=transformers.TrainingArguments(  
            ...
            optim="adamw_torch",  
            save_strategy="no",   
            output_dir="invalid_path",  # Use an empty string or ensure this is a valid path  
    ...
    trainer.get_grads(resume_from_checkpoint=resume_from_checkpoint)

    model.cpu()
    del model
    torch.cuda.empty_cache()
    
    # delete the folder constructed by trainer
    import shutil 
